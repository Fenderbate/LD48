...
uniform vec2 uv_scale = vec2(1f, 1f);
uniform vec2 uv_offset = vec2(0f, 0f);
...
void fragment() {
	ALBEDO.rgb = albedo.rgb * texture(albedo_texture, vec2(UV.x * uv_scale.x, UV.y * uv_scale.y) + uv_offset).rgb;
}
